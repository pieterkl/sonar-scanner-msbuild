...
function Get-MsBuildPath([ValidateSet("14.0", "15.0", "Current")][string]$msbuildVersion) {
    ...
    if (Test-Path $msbuild15Path) {
        Write-Debug "Found 'msbuild.exe' at '${msbuild15Path}'"
        return $msbuild15Path
    }

    throw "'msbuild.exe' located at '${msbuild15Path}' doesn't exist"
}
...
function Restore-Packages (
    [Parameter(Mandatory = $true, Position = 0)][ValidateSet("14.0", "15.0", "Current")][string]$msbuildVersion,
...
function Invoke-UnitTests([string]$binPath, [bool]$failsIfNotTest) {
    ...
    $testFiles = @()
    $testDirs = @()
    Get-ChildItem ".\sonar-scanner-msbuild\tests" -Recurse -Include "*Tests.dll" `
